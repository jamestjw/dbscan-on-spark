...
/** A naive implementation of DBSCAN. It has O(n2) complexity but uses no extra
  * memory. This implementation is not used by the parallel version of DBSCAN.
  */
class LocalDBSCANNaive(eps: Double, minPoints: Int) extends Logging {
  ...
  def fit(points: Iterable[DBSCANPoint]): Iterable[DBSCANLabeledPoint] = {
    ...
    val totalClusters =
      labeledPoints
        .foldLeft(DBSCANLabeledPoint.Unknown)((cluster, point) => {
          if (!point.visited) {
            point.visited = true
