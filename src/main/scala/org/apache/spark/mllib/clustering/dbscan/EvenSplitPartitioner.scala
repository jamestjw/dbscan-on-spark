...
class EvenSplitPartitioner(
  ...
  def findPartitions(
      toSplit: Set[RectangleWithCount]
  ): List[RectangleWithCount] = {
        ...
        if (count > maxPointsPerPartition) {
          ...
          if (canBeSplit(rectangle)) {
            ...
            def cost = (r: DBSCANRectangle) =>
              ((pointsIn(rectangle) / 2) - pointsIn(r)).abs
            ...
            logWarning(
              s"Can't split: ($rectangle -> $count) (maxSize: $maxPointsPerPartition)"
            )
  ...
  /** Returns the box that covers the space inside boundary that is not covered
    * by box
    */
  private def complement(
      box: DBSCANRectangle,
      boundary: DBSCANRectangle
  ): DBSCANRectangle =
      ...
      if (boundary.x2 >= box.x2 && boundary.y2 >= box.y2) {
        if (box.y2 == boundary.y2) {
          ...
          throw new IllegalArgumentException(
            "rectangle is not a proper sub-rectangle"
          )
        }
      } else {
  ...
  private def findPossibleSplits(box: DBSCANRectangle): Set[DBSCANRectangle] = {
    ...
    val xSplits = (BigDecimal(
      box.x + minimumRectangleSize
    ) until box.x2 by minimumRectangleSize).map(_.toDouble)
    val ySplits = (BigDecimal(
      box.y + minimumRectangleSize
    ) until box.y2 by minimumRectangleSize).map(_.toDouble)
  ...
  /** Returns true if the given rectangle can be split into at least two
    * rectangles of minimum size
    */
  ...
  def pointsInRectangle(
      space: Set[RectangleWithCount],
      rectangle: DBSCANRectangle
  ): Int = {
    space.view
      .filter({ case (current, _) => rectangle.contains(current) })
      .foldLeft(0) { case (total, (_, count)) =>
        total + count
      }
  }

  def findBoundingRectangle(
      rectanglesWithCount: Set[RectangleWithCount]
  ): DBSCANRectangle = {
    ...
    rectanglesWithCount.foldLeft(invertedRectangle) { case (bounding, (c, _)) =>
      DBSCANRectangle(
        bounding.x.min(c.x),
        bounding.y.min(c.y),
        bounding.x2.max(c.x2),
        bounding.y2.max(c.y2)
      )
    }
